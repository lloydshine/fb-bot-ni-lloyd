...
function ai(event, command, api) {
  ...
  const stopTyping = api.sendTypingIndicator(event.threadID, async (err) => {
    ...
    const userMessages =
      ...
      [        {          role: "system",          content: `You are an AI in a groupchat called ${            thread.threadName          }, You are programmed to engage in conversations, provide human-like responses. Also keep the response short`,        },        {          role: "assistant",          content: `Hello ${            data[event.senderID]["name"]
